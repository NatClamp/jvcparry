...
import {
  Text, Div, Row, Col, Container, Anchor, Input, Icon, Button,
} from 'atomize';
...
class DMsProductPage extends Component {
  ...
  searchTitles = async (term) => {
    if (term === '') {
      ...
      const products = await allProducts.filter((product) => {
        const title = product.title.toLowerCase().replace(':', ' ').replace('(', ' ').replace(')', ' ');
        // let singleTermMatch = title.split(" ").includes(lowercaseTerm)
        const multipleTermMatch = title.includes(lowercaseTerm);
        const completeMatch = title === lowercaseTerm;
        if (completeMatch) {
      ...
      this.setState({
        allProducts: products, currentPage: 1, totalPages, searchActive: true,
      }, () => {
  ...
  render() {
    const {
      allProducts, currentProducts, currentPage, totalPages, searchActive, searchValue,
    } = this.state;
        ...
        <Container>
          ...
          <Row d="flex" flexDir={{ xs: 'column', md: 'row' }} justify={{ xs: 'center', md: 'space-around' }}>
            ...
            <Col size={{ xs: '12', md: '2' }} d="flex" justify="center">
              {currentPage && (
              <Text tag="p" textWeight="300" textSize="body" textAlign="center" m={{ y: { xs: '1rem' } }}>
                Page
                {currentPage}
                {' '}
                /
            ...
            <Col size={{ xs: '12', md: '5' }} d="flex" align="center" justify={{ xs: 'center', md: 'flex-end' }}>
              ...
              {searchActive && (
              <Button d="flex" align="center" m={{ x: '1rem' }} p={{ x: '0.75rem' }} onClick={this.resetSearch} bg="white">
          ...
          <Row p={{ y: '2rem', x: '2rem' }}>
            <Col size={{ xs: '12', md: '7' }} d="flex" align="center" justify={{ xs: 'center', md: 'flex-start' }}>
              {currentPage && (
              <Text tag="p" textWeight="300" textSize="body" textAlign="center">
                Page
                {currentPage}
                {' '}
                /
